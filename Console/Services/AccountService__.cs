...
[Obsolete("Use AccountService instead.")]
public class AccountService__ : IAccountService
{
    private readonly Data.AppDbContext _context;

    public AccountService__(Data.AppDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<(bool Success, string Message, Models.Account? Account)> CreateAccountAsync(string userName, string accountNumber, decimal initialBalance)
    {
        ...
        if (initialBalance > 0)
        {
            var initialDeposit = new Models.Transaction
            {
                Account = account,
    ...
    public async Task<(bool Success, string Message)> TransferAsync(string fromAccountNumber, string toAccountNumber, decimal amount)
    {
        ...
        try
        {
            ...
            var withdrawalTx = new Models.Transaction
            {
            ...
            var depositTx = new Models.Transaction
            {
